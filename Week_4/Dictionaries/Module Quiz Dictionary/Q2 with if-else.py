	...
	for group,users in group_dictionary.items():
		...
		for user in users:
			if user not in user_groups:
				user_groups[user]= [] #creating dictionaries with user as key and empty lst for key (ex:: {admin: []})
			user_groups[user]+=  [group] # inserting groups as list values (ex:: {'admin': ['local', 'public', 'administrator']})
